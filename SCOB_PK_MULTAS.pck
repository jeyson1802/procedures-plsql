...
-----------------------------------------------------------
*/
procedure SCOB_SP_S_MONTO_MULTA_BY_CUM
(
  PI_VC_CUM           IN T_SCOB_MULTA.CUM%TYPE,
  po_cu_retorno       out cu_rec_set
);
...
-----------------------------------------------------------
*/
procedure SCOB_SP_U_MULTA_BY_MONTO(PI_V_ID_MULTA              IN VARCHAR2,
                                   PI_N_MI_CAPITAL            IN NUMBER,
                                   PI_N_MI_INTERES_COMP       IN NUMBER,
                                   PI_N_MI_INTERES_MORA       IN NUMBER,
                                   PI_N_MI_COSTAS             IN NUMBER,
                                   PI_N_TA_CAPITAL            IN NUMBER,
                                   PI_N_TA_INTERES_COMP       IN NUMBER,
                                   PI_N_TA_INTERES_MORA       IN NUMBER,
                                   PI_N_TA_COSTAS             IN NUMBER,
                                   PI_N_S_CAPITAL             IN NUMBER,
                                   PI_N_S_INTERES_COMP        IN NUMBER,
                                   PI_N_S_INTERES_MORA        IN NUMBER,
                                   PI_N_S_COSTAS              IN NUMBER,
                                   PI_V_NRO_FILE_AEC          IN VARCHAR2,
                                   PI_V_USUARIO_MOD           IN VARCHAR,
                                   po_n_retorno               out number,
                                   po_n_cod_error             out number,
                                   po_v_msj_error             out varchar2);
  ...
  -- ***************************************************************      
  PROCEDURE SCOB_SP_U_MULTA_BY_ESTADO(PIN_ID_MULTA         IN NUMBER,
                                      PI_N_COD_ESTADO_PAGO IN NUMBER,
                                      PI_V_USUARIO_MOD     IN VARCHAR,
                                      PO_N_RETORNO         OUT NUMBER,
                                      PO_N_COD_ERROR       OUT NUMBER,
                                      PO_V_MSJ_ERROR       OUT VARCHAR2);
...
--INICIO 12/07/2017 verifica si es un organo resolutivo de protección al consumidor el que intenta registrar una nueva multa, esto lo debe hacer en el SIRPC
  ...
  ELSE 
    ...
    SCOB_PK_RECAUDACION_BANCOS.SP_I_RECAUDACION_BN(PIN_NU_ID_MULTA => N_ID_MULTA,
                                                   POUT_NU_COD_RESULT => PO_N_RETORNO,
                                                   POUT_VC_MSG_RESULT => PO_V_MSJ_ERROR);
    ...
    SCOB_PK_RECAUDACION_BANCOS.SP_I_RECAUDACION_ASBANC(PIN_NU_ID_MULTA => N_ID_MULTA,
                                                       POUT_NU_COD_RESULT => PO_N_RETORNO,
                                                       POUT_VC_MSG_RESULT => PO_V_MSJ_ERROR);                                                                     
...
begin
  ...
  open po_cu_retorno for
      ...
      , DECODE(PE.RAZON_SOCIAL,NULL,PE.APE_PATERNO || ' ' || DECODE(PE.APE_MATERNO,NULL,PE.APE_CASADA,PE.APE_MATERNO) || 
        ' ' || PE.PRI_NOMBRE || ' ' || PE.SEG_NOMBRE, PE.RAZON_SOCIAL) AS NOMBRE
      , MD.VALOR_UIT 
      , MD.MONTO_UIT
      , T.COD_INTERNO
      , T.TXT_DESCRIPCION
      ...
      , MU.ID_MULTA
      , MU.NRO_FILE_AEC
      , ( SELECT SUM(CI.MONTO_EXCESO) FROM USR_SICOB.T_SCOB_COMPROBANTE_INGRESO CI 
          WHERE CI.ID_MULTA= MU.ID_MULTA AND CI.ESTADO='1'
          GROUP BY  CI.ID_MULTA) AS EXCESOS
      , (MU.MONTO_UIT + MU.MONTO_INTERES + MU.MONTO_INTERES_MORATORIO + MU.MONTO_COSTAS) AS MI_SUBTOTAL
      , (MU.TOTAL_AMORTIZADO + MU.TOTAL_AMORTIZADO_IC + MU.TOTAL_AMORTIZADO_IM + MU.TOTAL_AMORTIZADO_COSTAS) AS TA_SUBTOTAL
      ...
      FROM USR_SICOB.T_SCOB_MULTA MU INNER JOIN USR_SICOB.T_SCOB_MULTA_DETAL MD ON 
      (MU.ID_MULTA = MD.ID_MULTA AND MD.FLG_ACTUAL='1' AND MD.ESTADO='1')
      INNER JOIN USR_SICOB.T_SCOB_MULTA_SANCIONADO MS ON (MD.ID_MULTA_DETAL= MS.ID_MULTA_DETAL 
      AND MS.COD_TIPO_PERSONA='1' AND MS.FLG_PRINCIPAL='1' AND MS.ESTADO='1')
      INNER JOIN USR_SICOB.T_SCOB_PERSONA PE ON (MS.ID_PERSONA=PE.ID_PERSONA)
      LEFT JOIN USR_SICOB.T_SCOB_TABLA_DET T ON (MU.COD_ESTADO_PAGO=T.COD_INTERNO AND 
      T.ID_TABLA=34)
      LEFT OUTER JOIN USR_SICOB.T_SCOB_EXPEDIENTE_MULTA EM ON (MU.ID_MULTA = EM.ID_MULTA)
      LEFT OUTER JOIN USR_SICOB.T_SCOB_EXPEDIENTE EX ON (EM.ID_EXPEDIENTE = EX.ID_EXPEDIENTE)
...
BEGIN
      ...
      IF NVL(PI_V_NRO_FILE_AEC,' ') <> ' ' THEN
          po_n_retorno := -1;
          po_n_cod_error := po_n_retorno;
          po_v_msj_error := 'Ya se ha realizado el proceso de cobranza coactiva. No se puede modificar el registro.';
          return;
      END IF;
  
      
      UPDATE USR_SICOB.T_SCOB_MULTA MU 
      SET MU.MONTO_UIT = PI_N_MI_CAPITAL,
          MU.MONTO_INTERES = PI_N_MI_INTERES_COMP,
          MU.MONTO_INTERES_MORATORIO = PI_N_MI_INTERES_MORA,
          MU.MONTO_COSTAS = PI_N_MI_COSTAS,
          --POR CONFIRMAR SI VA
          --MU.MONTO_TOTAL_DEUDA = PI_N_MI_CAPITAL + PI_N_MI_INTERES_COMP + PI_N_MI_INTERES_MORA + PI_N_MI_COSTAS,
          MU.TOTAL_AMORTIZADO = PI_N_TA_CAPITAL,
          MU.TOTAL_AMORTIZADO_IC = PI_N_TA_INTERES_COMP,
          MU.TOTAL_AMORTIZADO_IM = PI_N_TA_INTERES_MORA,
          MU.TOTAL_AMORTIZADO_COSTAS = PI_N_TA_COSTAS,
          MU.SALDO = PI_N_S_CAPITAL,
          MU.SALDO_IC = PI_N_S_INTERES_COMP,
          MU.SALDO_IM = PI_N_S_INTERES_MORA,
          MU.SALDO_COSTAS = PI_N_S_COSTAS,
          MU.SALDO_TOTAL_DEUDA = PI_N_S_CAPITAL + PI_N_S_INTERES_COMP + PI_N_S_INTERES_MORA + PI_N_S_COSTAS,
          MU.AUDFECMODIFICACION = SYSDATE,
          MU.AUDUSUMODIFICACION = PI_V_USUARIO_MOD
      WHERE MU.ID_MULTA = PI_V_ID_MULTA;
      
      po_n_cod_error := 0;
      po_n_retorno := PI_V_ID_MULTA;
...
--
  ...
  BEGIN
    ...
    UPDATE USR_SICOB.T_SCOB_MULTA MU
       SET MU.COD_ESTADO_PAGO = PI_N_COD_ESTADO_PAGO,
           MU.AUDFECMODIFICACION = SYSDATE,
           MU.AUDUSUMODIFICACION = PI_V_USUARIO_MOD
     WHERE MU.ID_MULTA = PIN_ID_MULTA;

    PO_N_COD_ERROR := 0;
    PO_N_RETORNO   := PIN_ID_MULTA;
