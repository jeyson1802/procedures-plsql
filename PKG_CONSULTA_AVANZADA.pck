...
-- ******************************************************************************
  ...
  PROCEDURE SCOB_SP_S_MULTA_AVANZADA_NUEVO_2(
                                         -----------------------------------------------------------------------------------------------
                                         /*001*/PI_V_AREA_MACRO IN VARCHAR2 --AREA MACRO
                                         /*002*/,
                                         PI_V_CUM_INI IN VARCHAR2 --CUM
...
CREATE OR REPLACE PACKAGE BODY PKG_CONSULTA_AVANZADA IS
  ...
  BEGIN
    ...
    --PRIMERA INSTANCIA
       (SELECT DET.NRO_RESOLUCION
         ...
         WHERE DET.ESTADO = ''1''
           ...
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS NRO_RESOL_OR, --num_resolu
       (SELECT TO_DATE(TO_CHAR(DET.AUDFECCREACION, ''DD/MM/YYYY''),
                       ''DD/MM/YYYY'')
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 1
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_REG_OR,
       (SELECT DET.FEC_RESOLUCION
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 1
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_EMI_RESOL_OR, --fec_emision
       (SELECT DET.FEC_NOTIFICACION_RESOLUCION
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 1
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_NOTIF_RESOL_OR, --fec_notifica
       (SELECT DET.FEC_ESTADO_SGTE_INSTANCIA
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 1
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_APELACION,
       (SELECT DET.AUDUSUCREACION
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 1
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS USU_REG_OR,
       
       scob_pk_utilitario.scob_fn_dias_laborables_neg(to_char((SELECT DET.FEC_RESOLUCION
                                                                FROM T_SCOB_MULTA_DETAL DET
                                                               INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
                                                                  ON ORGA.ID_ORGANO_RESOLUTIVO =
                                                                     MDET.ID_ORGANO_RESOLUTIVO
                                                               WHERE DET.ESTADO = ''1''
                                                                 AND ORGA.TIPO_INSTANCIA = 1
                                                                 AND DET.ID_MULTA =
                                                                     MULT.ID_MULTA AND ROWNUM = 1),
                                                              ''DD/MM/YYYY''),
                                                      to_char((SELECT TO_DATE(TO_CHAR(DET.AUDFECCREACION,
                                                                                     ''DD/MM/YYYY''),
                                                                             ''DD/MM/YYYY'')
                                                                FROM T_SCOB_MULTA_DETAL DET
                                                               INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
    ...
    V_SQL_SELECT_2 := '
       ...
       (SELECT DET.NRO_RESOLUCION
         ...
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 2
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS NRO_RESOL_COMISION,
       (SELECT TO_DATE(TO_CHAR(DET.AUDFECCREACION, ''DD/MM/YYYY''),
                       ''DD/MM/YYYY'')
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
         ...
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 2
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_REG_COMISION,
       (SELECT DET.FEC_RESOLUCION
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 2
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_RESOL_COMISION,
       (SELECT DET.FEC_NOTIFICACION_RESOLUCION
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 2
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_NOTIF_RESOL_COMISION,
       (SELECT DET.FEC_ESTADO_SGTE_INSTANCIA
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 2
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_REVISION,
       (SELECT DET.AUDUSUCREACION
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 2
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS USU_REG_COMISION,
       ...
       (SELECT DET.NRO_RESOLUCION
         ...
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 3
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS NRO_RESOL_SALA,
       (SELECT TO_DATE(TO_CHAR(DET.AUDFECCREACION, ''DD/MM/YYYY''),
                       ''DD/MM/YYYY'')
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
         ...
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 3
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_REG_SALA,
       (SELECT DET.FEC_RESOLUCION
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 3
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_RESOL_SALA,
       (SELECT DET.FEC_NOTIFICACION_RESOLUCION
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 3
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS FEC_NOTIF_RESOL_SALA,
       (SELECT DET.AUDUSUCREACION
          FROM T_SCOB_MULTA_DETAL DET
         INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
            ON ORGA.ID_ORGANO_RESOLUTIVO = DET.ID_ORGANO_RESOLUTIVO
         WHERE DET.ESTADO = ''1''
           AND ORGA.TIPO_INSTANCIA = 3
           AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1) AS USU_REG_SALA,
    ...
    DECODE(MATE.CODIGO,11,DECODE(SUBM.CODIGO,21,TO_CHAR((SELECT DET.FEC_NOTIFICACION_RESOLUCION
                               FROM T_SCOB_MULTA_DETAL DET
                              INNER JOIN T_SCOB_ORGANO_RESOLUTIVO ORGA
                                 ON ORGA.ID_ORGANO_RESOLUTIVO =
                                    MDET.ID_ORGANO_RESOLUTIVO
                              WHERE DET.ESTADO = ''1''
                                AND ORGA.TIPO_INSTANCIA = 2
                                AND DET.ID_MULTA = MULT.ID_MULTA AND ROWNUM = 1),''DD/MM/YYYY''),''''),'''') AS FECH_NOT_SUNAT,
    --'''' AS FECH_NOT_SUNAT,
  ...
  WHERE MULT.ESTADO = ''1''';
    ...
    V_SQL_SELECT_1 || V_SQL_SELECT_2 || V_SQL_SELECT_6 || V_SQL_SELECT_3 || V_SQL_SELECT_4 || V_SQL_SELECT_5 || V_SQL_JOIN_1 || V_SQL_JOIN_2 || V_SQL_JOIN_3 || V_SQL_JOIN_4 || V_SQL_JOIN_5 || V_SQL_WHERE_1 || V_SQL_WHERE_2 || V_SQL_WHERE_3 || V_SQL_WHERE_4 || V_SQL_WHERE_5 || V_SQL_WHERE_6 || V_SQL_WHERE_7 || V_SQL_WHERE_8 || V_SQL_WHERE_9 || V_SQL_WHERE_10 || V_SQL_WHERE_11 || V_SQL_WHERE_12;
