CREATE OR REPLACE PACKAGE SCOB_PK_EXPEDIENTE IS
  ...
  */
  ...
  -- ***************************************************************      
  PROCEDURE SCOB_SP_S_ESTADOS_EXPEDIENTE(PI_V_NRO_EXPEDIENTE IN VARCHAR2,
                                         PO_CU_RETORNO       OUT CU_REC_SET,
                                         PO_N_COD_ERROR      OUT NUMBER,
                                         PO_V_MSJ_ERROR      OUT VARCHAR2);
  ...
  -- 30/12/2022     Victor Bendezú  Incidencias SICOB   
  -- ***************************************************************      
  PROCEDURE SCOB_SP_S_ESTADO_EXPEDIENTE(PO_CU_RETORNO  OUT CU_REC_SET,
                                        PO_N_COD_ERROR OUT NUMBER,
                                        PO_V_MSJ_ERROR OUT VARCHAR2);
  ...
  -- 30/12/2022     Victor Bendezú  Incidencias SICOB   
  -- ***************************************************************      
  PROCEDURE SCOB_SP_S_MOTIVO_EXPEDIENTE(PI_N_ID_ESTADO_EXPEDIENTE IN NUMBER,
                                        PO_CU_RETORNO             OUT CU_REC_SET,
                                        PO_N_COD_ERROR            OUT NUMBER,
                                        PO_V_MSJ_ERROR            OUT VARCHAR2);
  ...
  -- 30/12/2022     Victor Bendezú  Incidencias SICOB   
  -- ***************************************************************      
  PROCEDURE SCOB_SP_I_ESTADOS_EXPEDIENTE(PI_N_ID_ESTADO_EXP  IN NUMBER,
                                         PI_N_ID_MOTIVO      IN NUMBER,
                                         PI_N_ID_EXPEDIENTE  IN NUMBER,
                                         PI_N_ID_MULTA       IN NUMBER,
                                         PI_V_FECHA_REGISTRO IN VARCHAR,
                                         PI_V_USUARIO        IN VARCHAR,
                                         PO_N_COD_ERROR      OUT NUMBER,
                                         PO_V_MSJ_ERROR      OUT VARCHAR2);
  ...
  -- 30/12/2022     Victor Bendezú  Incidencias SICOB   
  -- ***************************************************************      
  PROCEDURE SCOB_SP_U_REVERTIR_EXPEDIENTE(PI_N_ID_EXPEDIENTE IN NUMBER,
                                          PI_V_USUARIO       IN VARCHAR,
                                          PO_N_COD_ERROR     OUT NUMBER,
                                          PO_V_MSJ_ERROR     OUT VARCHAR2);
  ...
  -- 30/12/2022     Victor Bendezú  Incidencias SICOB   
  -- ***************************************************************      
  PROCEDURE SCOB_SP_U_EMBARGO_EXPEDIENTE(PI_N_ID_EXPEDIENTE IN NUMBER,
                                         PI_V_FLG_EMBARGO   IN VARCHAR,
                                         PI_V_USUARIO       IN VARCHAR,
                                         PO_N_COD_ERROR     OUT NUMBER,
                                         PO_V_MSJ_ERROR     OUT VARCHAR2);
  ...
  -- 30/12/2022     Victor Bendezú  Incidencias SICOB   
  -- ***************************************************************      
  PROCEDURE SCOB_SP_S_CUMS_EXPEDIENTE(PI_ID_EXPEDIENTE IN NUMBER,
                                      PO_CU_RETORNO    OUT CU_REC_SET,
                                      PO_N_COD_ERROR   OUT NUMBER,
                                      PO_V_MSJ_ERROR   OUT VARCHAR2);
...
CREATE OR REPLACE PACKAGE BODY SCOB_PK_EXPEDIENTE IS
  ...
  */
  ...
  BEGIN
    ...
    OPEN PO_CU_RETORNO FOR 
      ...
      SELECT EXMU.ID_MULTA AS ID_MULTA,
       ...
       WHERE EXDO.ESTADO = '1'
         AND EX.ANIO || EX.NRO_EXPEDIENTE = PI_V_NRO_EXPEDIENTE
  ...
  BEGIN
    ...
    OPEN PO_CU_RETORNO FOR 
      ...
      SELECT T.ID_MOTIVO, T.CODIGO, T.DESCRIPCION, T.ESTADO
        FROM USR_SICOB.T_SCOB_EST_EXP_COB_MOTIVO T
       WHERE T.ID_EST_EXP_COB = PI_N_ID_ESTADO_EXPEDIENTE
         AND T.ESTADO = '1';
  ...
  V_ID_EXPEDIENTE_ESTADO T_SCOB_EXPEDIENTE_ESTADO.ID_EXPEDIENTE_ESTADO%TYPE;
  V_AUDFECCREACION       T_SCOB_EXPEDIENTE_ESTADO.AUDFECCREACION%TYPE;                                              
                                                
  BEGIN
    
     BEGIN
    
      SELECT REGI.AUDFECCREACION INTO V_AUDFECCREACION
        FROM (SELECT EXES.*
                FROM T_SCOB_EXPEDIENTE_ESTADO EXES
               WHERE EXES.ID_EXPEDIENTE = PI_N_ID_EXPEDIENTE
               ORDER BY 1 DESC) REGI
       WHERE ROWNUM = 1;
       
    EXCEPTION
      WHEN NO_DATA_FOUND THEN
        PO_V_MSJ_ERROR := 'NO CONFORME : NO HAY DATOS DE T_SCOB_EXPEDIENTE_ESTADO CON : ID_EXPEDIENTE => ' ||
                          PI_N_ID_EXPEDIENTE;
        RAISE EX_CONTROLADO;
      WHEN OTHERS THEN
        PO_V_MSJ_ERROR := 'NO CONFORME : SCOB_SP_I_ESTADOS_EXPEDIENTE - CONSULTA T_SCOB_EXPEDIENTE_ESTADO - ERROR : ' ||
                          SQLERRM;
        RAISE EX_CONTROLADO;
    END;  
    
    IF TO_DATE(PI_V_FECHA_REGISTRO,'DD/MM/YYYY') > TO_DATE(SYSDATE,'DD/MM/YY') THEN
      PO_V_MSJ_ERROR := 'LA FECHA NO PUEDE SER MAYOR A LA DE HOY';
      RAISE EX_CONTROLADO;
    END IF;
    
    IF V_AUDFECCREACION > TO_DATE(PI_V_FECHA_REGISTRO,'DD/MM/YYYY') THEN
      PO_V_MSJ_ERROR := 'LA FECHA NO PUEDE SER MENOR A LA ULTIMA FECHA REGISTRADA POR LOS ESTADOS DEL EXPEDIENTE';
      RAISE EX_CONTROLADO;
    END IF;
  
    V_ID_EXPEDIENTE_ESTADO := USR_SICOB.SCOB_SQ_EXPEDIENTE_ESTADO.NEXTVAL;
    ...
    INSERT INTO USR_SICOB.T_SCOB_EXPEDIENTE_ESTADO
      (ID_EXPEDIENTE_ESTADO,
       ...
       ID_MOTIVO)
    VALUES
      (V_ID_EXPEDIENTE_ESTADO,
       PI_N_ID_ESTADO_EXP,
       SCOB_PK_CONSTANTES.C_ESTADO_ACTIVO,
       TO_DATE(PI_V_FECHA_REGISTRO,'DD/MM/YYYY'),
       PI_V_USUARIO,
       PI_N_ID_EXPEDIENTE,
       PI_N_ID_MOTIVO);
     
    UPDATE USR_SICOB.T_SCOB_EXPEDIENTE
       SET ID_MOTIVO             = PI_N_ID_MOTIVO,
           ESTADO_EXPEDIENTE     = PI_N_ID_ESTADO_EXP,
           FEC_ESTADO_EXPEDIENTE = TO_DATE(PI_V_FECHA_REGISTRO,'DD/MM/YYYY'),
           AUDFECMODIFICACION    = SYSDATE,
           AUDUSUMODIFICACION    = PI_V_USUARIO
  ...
  CURSOR CURSOR_T_SCOB_EXPEDIENTE_MULTA IS
   SELECT EM.*
    ...
    WHERE EM.ID_EXPEDIENTE = PI_N_ID_EXPEDIENTE
    AND EM.ESTADO = '1';
  ...
  BEGIN
    ...
    IF REG_T_SCOB_EXPEDIENTE.ESTADO_EXPEDIENTE <> 1 AND REG_T_SCOB_EXPEDIENTE.ESTADO_EXPEDIENTE <> 3 THEN
      PO_V_MSJ_ERROR := 'ESTADO DEL EXPEDIENTE DEBE SER PRE-COACTIVO O EN EJECUCION COACTIVA';
      RAISE EX_CONTROLADO;
    END IF;
    ...
    IF TO_CHAR(REG_T_SCOB_EXPEDIENTE.FEC_INGRESO, 'mmyyyy') <> TO_CHAR(SYSDATE, 'mmyyyy')  THEN
      PO_V_MSJ_ERROR := 'SOLO PUEDE REVERTIR EN EL MISMO MES DE LA GENERACION DEL EXPEDIENTE';
      RAISE EX_CONTROLADO;
    END IF;
    
    -- SE OBTIENE EL ULTIMO ESTADO EXPEDIENTE ACTIVO
    SELECT REGI.ID_EXPEDIENTE_ESTADO INTO V_ID_EXPEDIENTE_ESTADO
        FROM (SELECT EXES.*
                FROM T_SCOB_EXPEDIENTE_ESTADO EXES
               WHERE EXES.ID_EXPEDIENTE = PI_N_ID_EXPEDIENTE
               AND EXES.ESTADO = '1'
    ...
    UPDATE USR_SICOB.T_SCOB_EXPEDIENTE_ESTADO EXES
       SET EXES.ESTADO             = '0',
           EXES.AUDFECMODIFICACION = SYSDATE,
           EXES.AUDUSUMODIFICACION = PI_V_USUARIO
     WHERE EXES.ID_EXPEDIENTE = PI_N_ID_EXPEDIENTE;    
    ...
    UPDATE USR_SICOB.T_SCOB_EXPEDIENTE EXPE
       SET EXPE.ESTADO             = '0',
           EXPE.AUDFECMODIFICACION = SYSDATE,
           EXPE.AUDUSUMODIFICACION = PI_V_USUARIO
     WHERE EXPE.ID_EXPEDIENTE = PI_N_ID_EXPEDIENTE;
    ...
    IF REG_T_SCOB_EXPEDIENTE.MONTO_COSTAS > 0 THEN
      ...
      UPDATE USR_SICOB.T_SCOB_EXPEDIENTE_COSTA EXCO
         SET EXCO.ESTADO             = '0',
             ...
             EXCO.AUDUSUMODIFICACION = PI_V_USUARIO
       WHERE EXCO.ID_EXPEDIENTE = PI_N_ID_EXPEDIENTE;
      ...
      LOOP 
        ...
        UPDATE USR_SICOB.T_SCOB_MULTA_ESTADO MUES
           SET MUES.ESTADO             = '0',
               ...
               MUES.AUDUSUMODIFICACION = PI_V_USUARIO
         WHERE MUES.ID_MULTA = REG_T_SCOB_EXPEDIENTE_MULTA.ID_MULTA
           AND MUES.ESTADO_MULTA > 9;
        
        -- ACTUALIZAR EL ESTADO DE LA MULTA A NO INICIADO CON EL MOTIVO 
        UPDATE USR_SICOB.T_SCOB_MULTA MULT
    ...
    UPDATE USR_SICOB.T_SCOB_EXPEDIENTE_MULTA EXMU
       SET EXMU.ESTADO             = '0',
           EXMU.AUDFECMODIFICACION = SYSDATE,
           EXMU.AUDUSUMODIFICACION = PI_V_USUARIO
     WHERE EXMU.ID_EXPEDIENTE = PI_N_ID_EXPEDIENTE;
 ...
 END;
  ...
  BEGIN
    ...
    UPDATE USR_SICOB.T_SCOB_EXPEDIENTE EX
       SET EX.FLG_EMBARGO = PI_V_FLG_EMBARGO,
           EX.AUDFECMODIFICACION = SYSDATE,
           EX.AUDUSUMODIFICACION = PI_V_USUARIO
     WHERE EX.ID_EXPEDIENTE = PI_N_ID_EXPEDIENTE;
  ...
  EXCEPTION
    WHEN OTHERS THEN
      ...
      SELECT CASE WHEN PO_N_COD_ERROR > 0 THEN PO_N_COD_ERROR * -1 ELSE PO_N_COD_ERROR END
        INTO PO_N_COD_ERROR
        FROM DUAL;
      PO_V_MSJ_ERROR := SUBSTR(SQLERRM, 1, 250) || ' - ' || TO_CHAR(PI_N_ID_EXPEDIENTE);
      ROLLBACK;
    ...
    OPEN PO_CU_RETORNO FOR 
     ...
     SELECT *
       FROM (SELECT EX.ANIO || EX.NRO_EXPEDIENTE || ' - ' || MU.ANIO || MU.CUM AS EXP_ACUM,
              ...
              WHERE SA.ID_EXP_DESTINO = PI_ID_EXPEDIENTE
             UNION ALL
