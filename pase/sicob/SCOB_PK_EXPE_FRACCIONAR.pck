...
CREATE OR REPLACE PACKAGE BODY SCOB_PK_EXPE_FRACCIONAR IS
    ...
    BEGIN
        ...
        IF PO_N_RETORNO < 0 THEN
           ...
           RETURN;
        END IF;
        USR_SICOB.PKG_RECAUDACION_BANCOS.SP_DO_GRABAR_BN(PIN_NU_ID_MULTA => N_ID_MULTA,
                                     POUT_NU_COD_RESULT => PO_N_RETORNO,
                                     POUT_VC_MSG_RESULT => PO_V_MSJ_ERROR);
        IF PO_N_RETORNO < 0 THEN
           ROLLBACK;
           RETURN;
        END IF;
        USR_SICOB.PKG_RECAUDACION_BANCOS.SP_DO_GRABAR_ASBANC(PIN_NU_ID_MULTA => N_ID_MULTA,
                                         POUT_NU_COD_RESULT => PO_N_RETORNO,
                                         POUT_VC_MSG_RESULT => PO_V_MSJ_ERROR);
        IF PO_N_RETORNO < 0 THEN
           ROLLBACK;
